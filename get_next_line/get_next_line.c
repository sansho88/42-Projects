...
/*   Updated: 2021/12/06 11:29:04 by tgriffit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if ((fd < 0 || fd > 1023) || BUFFER_SIZE <= 0)
		return (NULL);
	if (read(fd, vars.buffer, 0) < 0)
		return (NULL);
	text = ft_strdup("");
	if (!text)
		return (NULL);
	vars.bytes = 1;
	while (!ft_strnstr(text, "\n", ft_strlen(text)) && vars.bytes > 0)
	{
		reader(fd, &text, &vars);
		if (!text)
			return (0);
	}
	if (ft_strnstr(text, "\n", ft_strlen(text)) || (vars.bytes == 0 && *text))
		return (get_line(text, &vars.rmndr));
	free(text);
	return (NULL);
}

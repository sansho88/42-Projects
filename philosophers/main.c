...
void	myusleep(useconds_t time)
{
	...
	start = temps.tv_usec;
	timestamp =(temps.tv_sec * 1000000 + temps.tv_usec)- (start * 1000000 + start);
	while (timestamp - start < time)//(temps.tv_usec - start < time)
	{
		gettimeofday(&temps, NULL);
		timestamp =(temps.tv_sec * 1000000 + temps.tv_usec)- (start * 1000000 + start);
		usleep(1);
	}
	//dprintf(2, "[%s] time(%ld) - start(%u) = %zu\n", __func__,temps.tv_usec, start, temps.tv_usec - start);
}

void	print_act(t_philo philosoph, const char *act, bool finish)
{
	useconds_t		timestamp;
	...
	timestamp = ((realtime.tv_sec * 1000 + realtime.tv_usec / 1000)
						- (philosoph.birth.tv_sec * 1000 + philosoph.birth.tv_usec / 1000));//realtime.tv_usec / 1000 - philosoph.birth.tv_usec / 1000;
	dprintf(2, "[BIRTH]%ld\n", philosoph.birth.tv_usec);
	if (finish == false)
	{
...
bool	eat(t_philo *philo, useconds_t time)
{
	...
	printf("[%ldms]\t%zu has taken his fork (%p)\n",
		   (temps.tv_sec * 1000 + temps.tv_usec) - (philo->birth.tv_sec * 1000 + philo->birth.tv_usec), philo->id, &philo->fork);
	...
	printf("[%ldms]\t%zu has taken an other fork (%p)\n",
		   (temps.tv_sec * 1000 + temps.tv_usec) - (philo->birth.tv_sec * 1000 + philo->birth.tv_usec), philo->id, philo->nextfork);
	nb_locks += 2;
	...
	myusleep(time);
	if ((temps.tv_usec - philo->lastmeal.tv_usec) >= philo->starve)
	{
		nb_locks -= 2;
		philo->isalive = false;
		printf("[%ldms]\t%d died (time vs starve = %ld vs %ld)\n", ((temps.tv_sec * 1000000 + temps.tv_usec)
																					- (philo->birth.tv_sec * 1000000 + philo->birth.tv_usec)),
...
void	*routine(void *philosoph)
{
	...
	printf("[%ldms]\troutine of philo[%d]\n", (time.tv_usec - philo.birth.tv_usec),
		   (int)philo.id);
	while (time.tv_usec - philo.birth.tv_usec >= philo.lifetime && philo.isalive)
	{
...
void	chain_philos(size_t nb_philos, t_philo *cavern, int argc, char **argv)
{
	...
	*everything_is_ok = true;
	while (cuffs < nb_philos)
	{
		...
		cavern[cuffs].nextphilo = &cavern[(cuffs + 1) % nb_philos];

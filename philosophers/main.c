...
/*   Updated: 2022/04/12 13:54:25 by tgriffit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	while (i < nb_philos)
	{
		//cavern[i] = (t_philo *)malloc(sizeof(t_philo));
		cavern[i].id = i + 1;
		cavern[i].starve = 10;
		cavern[i].fork.isfree = true;
		cavern[i].isalive = true;
		gettimeofday(&cavern[i].lastmeal, NULL);
		if (i < nb_philos - 1)
